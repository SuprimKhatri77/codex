...
export const metadata: Metadata = {
  title: "Auth",
  description: "auth notes — authentication, authorization, best practices",
};

export default function AuthPage() {
  const all = getAllNotes();
  const authNotes = all.filter((n) => n.href.startsWith("/auth"));

  const grouped = authNotes.reduce<Record<string, Note[]>>((acc, note) => {
    ...
    <main className="min-h-screen bg-cx text-cx-fg px-8 py-12 max-w-4xl mx-auto">
      <div className="mb-12">
        ...
        <h1 className="text-4xl font-light tracking-tight text-cx-fg mb-2">
          auth <span className="font-medium text-cx-heading">notes</span>
        </h1>
