...
export const metadata: Metadata = {
  title: "Go",
  description: "go notes — syntax, patterns, things that clicked",
};

export default function GolangPage() {
  const all = getAllNotes();
  const goNotes = all.filter((n) => n.href.startsWith("/golang"));

  const grouped = goNotes.reduce<Record<string, Note[]>>((acc, note) => {
    ...
    <main className="min-h-screen bg-cx text-cx-fg px-8 py-12 max-w-4xl mx-auto">
      <div className="mb-12">
        ...
        <h1 className="text-4xl font-light tracking-tight text-cx-fg mb-2">
          go <span className="font-medium text-cx-heading">notes</span>
        </h1>
