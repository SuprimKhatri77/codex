...
export const metadata: Metadata = {
  title: "Docker",
  description: "docker notes — containerization, orchestration, networking",
};

export default function DockerPage() {
  const all = getAllNotes();
  const dockerNotes = all.filter((n) => n.href.startsWith("/docker"));

  const grouped = dockerNotes.reduce<Record<string, Note[]>>((acc, note) => {
    ...
    <main className="min-h-screen bg-cx text-cx-fg px-8 py-12 max-w-4xl mx-auto">
      <div className="mb-12">
        ...
        <h1 className="text-4xl font-light tracking-tight text-cx-fg mb-2">
          docker <span className="font-medium text-cx-heading">notes</span>
        </h1>
